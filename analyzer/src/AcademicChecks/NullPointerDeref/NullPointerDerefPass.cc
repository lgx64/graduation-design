...
static NullState evalPtrState(Value *V, const Env &env, int depth = 0) {//输入一个指针值V和当前路径环境env，输出它在此路径下的状态
    ...
    if (auto *LI = dyn_cast<LoadInst>(V)) {
        ...
        return evalPtrState(LI->getPointerOperand(), env, depth + 1);
    }

    return maybeNullFallback(V) ? NullState::Unknown : NullState::NonNull;
}
...
static void processInstructionForState(Instruction &I, Env &env) {//状态转移函数
    ...
    if (auto *LI = dyn_cast<LoadInst>(&I)) {
        if (LI->getType()->isPointerTy()) {
            envSet(env, LI, evalPtrState(LI->getPointerOperand(), env));
        }
        return;
    }
