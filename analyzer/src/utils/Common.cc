...
#define LINUX_SOURCE "YOUR KERNEL SOURCE PATH"
...
// get source code and pre-comments of a function.
string getFunctionSourceCode(Function *F){
    ...
    string FN = getFileName(NULL, SP);
    unsigned start_line_number = SP->getLine();

    string pre_start_line = getSourceLine(FN, start_line_number - 1);
    if (hasSubString(pre_start_line, "*/")) {
        start_line_number -= 2;
        string current_start_line = getSourceLine(FN, start_line_number);
        while(!hasSubString(current_start_line, "/*")){
            start_line_number--;
            current_start_line = getSourceLine(FN, start_line_number);
    ...
    unsigned end_line_number = -1;
    for (Instruction& I : instructions(F)) {
        ReturnInst *RI = dyn_cast<ReturnInst>(&I);
        if (!RI) { continue; }

        DILocation *Loc = getSourceLocation(RI);
        if (!Loc) { return ""; }
        
        end_line_number = Loc->getLine();
        break;
    }
    if(end_line_number == -1) { return ""; }
    
    string func_code = "";
    for(int i = start_line_number; i != end_line_number+1; i++) {
        func_code += getSourceLine(FN, i) + "\n";
    }
    return func_code;
}
